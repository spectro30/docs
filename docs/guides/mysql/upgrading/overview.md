...
This guide will give you an overview on how KubeDB enterprise operator upgrade the version of `MySQL` group replication.
...
The following diagram shows how KubeDB enterprise operator used to upgrade the version of `MySQL` group replication. Open the image in a new tab to see the enlarged version.
...
4. Then, in order to upgrade the version of the `MySQL` database the user creates a `MySQLOpsRequest` cr with desired the desired version.
